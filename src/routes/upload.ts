...
export async function uploadFolderRoute(
  ...
  for (const file of files) {
    ...
    // For folder uploads, file.name contains the relative path (e.g., "folder/subfolder/file.txt")
    // Normalize path separators (convert backslashes to forward slashes)
    const relativePath = file.name.replace(/\\/g, "/").trim();
    if (!relativePath) continue;

    // Build the file key by combining the base path with the relative path
